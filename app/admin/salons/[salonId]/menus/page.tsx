...
export default function AdminSalonMenusPage() {
  ...
  const fetchMenus = async () => {
    try {
      const q = query(collection(db, 'salons', salonId, 'menus'), orderBy('order', 'asc'));
      const snapshot = await getDocs(q);
      const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() }) as Menu);
      setMenus(data);
    } catch {
      const snapshot = await getDocs(collection(db, 'salons', salonId, 'menus'));
      const data = snapshot.docs.map(d => ({ id: d.id, ...d.data() }) as Menu);
      setMenus(data);
    } finally {
      setLoading(false);
  ...
  const resetForm = () => {
    setForm({ name: '', time: '', price: '', description: '', isActive: true, order: menus.length });
    setEditMenu(null);
    setShowForm(false);
  };

  const handleEdit = (menu: Menu) => {
    setForm({ name: menu.name, time: menu.time, price: menu.price, description: menu.description, isActive: menu.isActive, order: menu.order });
    setEditMenu(menu);
    setShowForm(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editMenu) {
        await updateDoc(doc(db, 'salons', salonId, 'menus', editMenu.id), { ...form });
        alert('メニューを更新しました');
      } else {
        await addDoc(collection(db, 'salons', salonId, 'menus'), {
          ...form,
          order: menus.length,
          createdAt: new Date(),
        });
  ...
  const handleToggleActive = async (menu: Menu) => {
    await updateDoc(doc(db, 'salons', salonId, 'menus', menu.id), { isActive: !menu.isActive });
    fetchMenus();
  };

  const handleDelete = async (id: string) => {
    if (!confirm('このメニューを削除しますか？')) return;
    await deleteDoc(doc(db, 'salons', salonId, 'menus', id));
    fetchMenus();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <AdminHeader
        title="メニュー管理"
        subtitle={salonName ? `${salonName} ・ ${menus.length}件` : `${menus.length}件登録`}
      ...
      {showForm && (
        <div className="m-4 bg-white rounded-xl shadow p-4">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-sm font-bold text-gray-700">メニュー名 <span className="text-red-500">*</span></label>
              <input type="text" placeholder="例：カット" value={form.name}
                onChange={e => setForm({ ...form, name: e.target.value })}
                className="w-full border rounded-lg p-2 mt-1" />
            </div>
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-sm font-bold text-gray-700">所要時間 <span className="text-red-500">*</span></label>
                <input type="text" placeholder="例：60分" value={form.time}
                  onChange={e => setForm({ ...form, time: e.target.value })}
                  className="w-full border rounded-lg p-2 mt-1" />
              </div>
              <div className="flex-1">
                <label className="text-sm font-bold text-gray-700">料金 <span className="text-red-500">*</span></label>
                <input type="text" placeholder="例：¥4,000" value={form.price}
            ...
            <div>
              ...
              <textarea placeholder="例：シャンプー・ブローが含まれます" value={form.description}
                onChange={e => setForm({ ...form, description: e.target.value })}
                rows={2} className="w-full border rounded-lg p-2 mt-1 text-sm" />
            </div>
            <div>
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="isActive" checked={form.isActive}
                ...
                className="w-4 h-4" />
              <label htmlFor="isActive" className="text-sm font-bold text-gray-700">表示する</label>
            </div>
          ...
          menus.map(menu => (
            <div key={menu.id} className="bg-white rounded-xl shadow p-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full font-bold ${
                      menu.isActive ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'
                    }`}>
              ...
              <div className="flex gap-2 border-t pt-3">
                ...
                <button onClick={() => handleToggleActive(menu)}
                  className={`flex-1 rounded-lg py-2 text-sm font-bold border ${
                    menu.isActive ? 'border-gray-300 text-gray-600' : 'border-green-500 text-green-600'
                  }`}>
