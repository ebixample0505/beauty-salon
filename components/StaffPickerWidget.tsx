      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
        <button
          ...
          className={`shrink-0 px-4 py-3 rounded-full border-2 font-bold text-sm cursor-pointer ${
            selectedStaffId === ''
              ? 'border-accent bg-main-light text-accent'
            ...
            <div className="mt-2">
              {showFullCareer ? (
                <>
                  ...
                  <button onClick={() => setShowFullCareer(false)} className="text-xs text-accent font-bold mt-1 cursor-pointer">
                    閉じる
                  </button>
                </>
              ) : (
                <button onClick={() => setShowFullCareer(true)} className="text-xs text-accent font-bold underline cursor-pointer">
                  詳細を見る
                </button>
              )}
            </div>
